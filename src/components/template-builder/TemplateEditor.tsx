  ...
  renderHTML({ HTMLAttributes }) {
    return [
      'div',
      {
        ...
        class: 'p-2 border rounded bg-gray-100 flex items-center gap-2',
      },
      ...(HTMLAttributes.imageUrl
        ? [['img', { src: HTMLAttributes.imageUrl, class: 'h-8 w-auto', alt: HTMLAttributes.label }]]
        : []),
      ['span', { class: 'text-sm font-medium text-gray-700' }, HTMLAttributes.label],
    ];
  },
});
    ...
    extensions: [
      ...
      ...(provider ? [
        CollaborationCursor.configure({ 
          provider, 
          user: { name: userName, color: '#8a7fae' } 
        }),
      ] : []),
      ...
      // TODO: Add Comments extension when ready
      // Comments.configure({
      //   HTMLAttributes: {
      //     class: 'comment',
      //   },
  ...
  return (
    <div className="template-editor-container flex flex-col h-full bg-white rounded-lg shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto relative">
        <EditorContent editor={editor} id="editor-droppable" className="template-editor" />
      </div>
      <div className="border-t border-gray-200 px-4 py-2 flex items-center justify-between text-sm text-gray-600">
        <div className="flex items-center space-x-4">
          ...
          <span className="text-purple-600">💬 Ask Casper AI to help</span>
        </div>
